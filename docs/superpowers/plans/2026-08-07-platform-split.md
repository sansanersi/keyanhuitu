...
- 模型策略先小后大：先用本地小模型走通流程，验证 RAG / 文档 / 图片 / 生成图链路，数据量变大后再迁移到 305 等更强机器并更换更大模型。
...
2. 再加服务聚合层，让路由依赖新边界。
3. 再加 MySQL 逻辑库配置抽象。
4. 保持 Ollama 小模型作为第一阶段默认模型入口。
5. 最后规划数据迁移，不在第一批代码里直接迁移真实数据。
...
SCI_MODEL_ROLLOUT_STAGE=local_small_model
```
